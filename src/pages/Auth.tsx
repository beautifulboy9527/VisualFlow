...
const Auth: React.FC = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (error) {
        toast({
          title: language === 'zh' ? 'Google 登录失败' : 'Google Sign In Failed',
          description: error.message,
          variant: 'destructive',
        });
      }
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="bg-card/90 backdrop-blur-xl rounded-2xl border border-border/50 p-8 shadow-lg">
          ...
          <div className="relative my-6">
            <Separator />
            <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-card px-3 text-xs text-foreground-muted">
              {language === 'zh' ? '或使用邮箱' : 'or continue with email'}
            </span>
          </div>
